...
const compiled = await build({
  ...
  plugins:[{name:'fake-io',setup(b){
    ...
    b.onLoad({filter:/_shared\/spend\.ts$/},()=>({contents:`export const INTERNAL_CAP_EXCEEDED='cap',PROVIDER_QUOTA_EXCEEDED='quota',LOCAL_ONLY_BLOCKED='LOCAL_ONLY_BLOCKED';export async function readLocalOnlyMode(){return {enabled:false}};export const looksLikeProviderQuota=()=>false;export async function checkCap(){return {blocked:false,ledger:{usd:0},capUsd:35}};export async function recordSpend(){}`}));
