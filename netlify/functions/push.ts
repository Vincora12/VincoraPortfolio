...
import { removePushSubscription, savePushSubscription, type StoredPushSubscription } from './_shared/pushDelivery';

export default async function push(request: Request): Promise<Response> {
  if (!authorize(request).ok) return denied();
  if (request.method === 'DELETE') {
    const body = await request.json().catch(() => null) as { endpoint?: string } | null;
    if (!body?.endpoint) return json({ error: 'endpoint non valido' }, 400);
  ...
  if (request.method !== 'POST') return json({ error: 'solo GET, POST e DELETE' }, 405);
  ...
  await savePushSubscription(subscription as StoredPushSubscription);
  return json({ ok: true });
}
