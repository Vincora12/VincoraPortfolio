...
type ResponsesInputItem = { role?: string; content?: unknown } | string;
...
export default async function handler(request: Request): Promise<Response> {
  ...
  const mapped = mapMessagesToRequest(messages);
  const tools = mapResponsesTools(payload.tools);
  const outcome = await runIngress(request, mapped, tools);
