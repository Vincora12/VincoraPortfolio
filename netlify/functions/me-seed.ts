...
export default async function handler(request: Request): Promise<Response> {
  ...
  const result = await importPersonalMemorySeed(body.seed, async (seed) => {
    ...
    /* ⚠️ Era l'unica chiamata LLM di questo dominio a non registrare mai la spesa — vedi la
       nota in core/memory.ts. Avvolta in try/catch per lo stesso motivo: un guasto del
       registro spesa non deve trasformare un import Seed riuscito in un fallimento riportato. */
    if (response.usage.inputTokens || response.usage.outputTokens) {
      try { await recordSpend('text-cheap', response.model, response.usage, { action: 'me_seed', subsystem: 'memory' }); } catch { /* telemetry only */ }
    }
