...
/* vNext MODEL GATEWAY — the price table moved to `./prices.ts`, the ONE
   list shared by this ledger and by the browser's estimates (src/ai/usage.ts),
   which used to keep a second copy that drifted. */
import { MODEL_PRICES as PRICES, type ModelPrice as Price } from './prices';
