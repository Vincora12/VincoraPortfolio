...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!event || !(['CLIENT_RUNTIME_ERROR', 'CHAT_SEND_START', 'CHAT_RESPONSE_OK', 'CHAT_RESPONSE_ERROR', 'CHAT_UI_SUBMIT', 'CHAT_MODEL_ADAPTER_START', 'CHAT_BASE_MODEL_START', 'CHAT_MEMORY_FETCH_START', 'CHAT_AI_FETCH_START', 'CHAT_CLIENT_ERROR', 'STORAGE_LOCAL_WRITE_START', 'STORAGE_LOCAL_WRITE_OK', 'STORAGE_CLIENT_ERROR', 'LOCAL_STORAGE_WRITE_START', 'LOCAL_STORAGE_WRITE_OK', 'LOCAL_STORAGE_WRITE_ERROR'] as string[]).includes(event.eventType)) return json({ error: 'evento non valido' }, 400);
