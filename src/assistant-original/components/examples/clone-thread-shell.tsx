...
export const CloneThreadShell: FC<CloneThreadShellProps> = ({
  ...
  const touchStart = useRef<{ x: number; y: number; target: EventTarget | null; axis: 'pending' | 'x' | 'y'; open: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    const start = (event: globalThis.TouchEvent) => {
      ...
      if (!mobileOpen && event.target instanceof Element && event.target.closest('.vinz-conversation-tabs')) return;
      const touch = event.touches[0];
      touchStart.current = { x: touch.clientX, y: touch.clientY, target: event.target, axis: 'pending', open: mobileOpen };
    };
    const move = (event: globalThis.TouchEvent) => {
      ...
      if (initial.axis === 'pending') {
        const horizontal = Math.abs(dx) > Math.abs(dy) * 1.15;
        const intendedDirection = initial.open ? dx < 0 : dx > 0;
        initial.axis = horizontal && intendedDirection ? 'x' : 'y';
      }
      if (initial.axis !== 'x') return;
      event.preventDefault();
      if (!initial.open) setMobileOpen(true);
      const distance = initial.open
        ? Math.min(width(), Math.max(0, width() + dx))
      ...
      {mobileOpen && (
        ...
        >
          ...
          >
            ...
            {showThreadList && <>
              <div className="vinz-project-page__section">Recenti</div>
              <ThreadListRoot className="min-h-0 pb-4">
