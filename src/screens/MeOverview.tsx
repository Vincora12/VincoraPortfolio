...
export function MeOverviewScreen({ onGo: _onGo }: { onGo: (o: Overlay) => void }) {
  ...
  return <div className="screen me-health">
    <header className="me-health__header"><div><h1>ME</h1><p>{new Intl.DateTimeFormat('it-IT', { weekday: 'long', day: 'numeric', month: 'long' }).format(new Date())}</p></div><button type="button" aria-label="Aggiungi con AI" onClick={() => askAi('Voglio aggiornare la mia salute: ')}>＋</button></header>
    ...
    {journal.display.goal && <button type="button" className="me-health__goal" onClick={() => askAi(`Parliamo del mio obiettivo: ${journal.display.goal}`)}><span>OBIETTIVO DEL PERIODO</span><strong>{journal.display.goal}</strong></button>}
