...
/** Aggiorna la registrazione scelta dalla UI, senza affidarsi al concetto di “ultima”. */
export function updateMealById(
  entryId: string,
  patch: Partial<Pick<MealLog, 'description' | 'kcal' | 'protein' | 'carbs' | 'fat'>>,
): boolean {
  const journal = readHealthJournal();
  if (!journal.meals.some((meal) => meal.id === entryId)) return false;
  save({
    ...journal,
    meals: journal.meals.map((meal) => meal.id === entryId ? { ...meal, ...patch } : meal),
  });
  return true;
}
...
export function updateWorkoutById(
  ...
  save({
    ...journal,
    workouts: journal.workouts.map((workout) => workout.id === entryId ? { ...workout, ...patch } : workout),
  });
  return true;
}
