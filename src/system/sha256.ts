...
function sha256WithoutWebCrypto(bytes: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let i = 0; i < 64; i += 1) {
      ...
      const temp1 = (h + sum1 + choice + ROUND[i] + words[i]) >>> 0;
...
/** SHA-256 also on HTTP LAN/Tailscale origins where WebCrypto is unavailable. */
export async function sha256Hex(value: string): Promise<string> {
  const bytes = new TextEncoder().encode(value);
  const subtle = globalThis.crypto?.subtle;
  if (subtle) return toHex(new Uint8Array(await subtle.digest('SHA-256', bytes)));
  return toHex(sha256WithoutWebCrypto(bytes));
}
